...
function GuideBoard({
  ...
  const items: Array<{
  ...
  }> = [
    {
      ...
      detail: "Login/parol bilan kirgach, AI orqali yuzni bir marta to‘liq ulash",
    },
    {
      id: "permission",
      n: 1,
    ...
    },
    {
      ...
      detail: "AI shu akkaunt egasining yuzini solishtiradi — boshqa odam o‘tmaydi",
    },
    {
      id: "keldim",
      n: 3,
...
export default function DavomatFacePage() {
  ...
  /** Face ID davomat faqat login/parol bilan kirgandan keyin */
  useEffect(() => {
    if (isLoading) return;
    if (!isAuthenticated) {
      setLocation("/login");
  ...
  /**
   * Face ID skani — avval login majburiy, keyin shu akkaunt yuzi.
  ...
  const faceLockedReason = useMemo(() => {
    if (!isAuthenticated) return "Avval login va parol bilan tizimga kiring";
  ...
  const onCaptured = async (
    ...
    if (!isAuthenticated) {
      setLocation("/login");
      throw new Error("Avval login va parol bilan kiring");
    }
        ...
        description:
          result.nextAction === "done"
            ...
            : result.nextAction === "out"
              ? "Yuz tasdiqlandi — bu siz. Faqat Ketdim qolgan"
              : "Yuz tasdiqlandi — bu siz. Endi Keldim ni bosing",
      });
      return { fullName: result.fullName };
    } catch (err) {
      if (err instanceof DavomatApiError && err.code === "face_not_account_owner") {
        const accountName = err.fullName || user?.fullName || "Bu akkaunt";
        const text = `${accountName} — siz emassiz`;
        toast({
          title: text,
          description: "Bu akkaunt egasining yuzi bilan mos kelmadi. Boshqa odam Face ID qila olmaydi.",
          variant: "destructive",
        });
  ...
  const punch = async (action: "in" | "out") => {
    ...
    try {
      ...
      if (err instanceof DavomatApiError && err.code === "face_not_account_owner") {
        const accountName = err.fullName || user?.fullName || "Bu akkaunt";
        toast({
          title: `${accountName} — siz emassiz`,
          description: "Bu akkaunt egasining yuzi bilan mos kelmadi.",
          variant: "destructive",
        });
        return;
      }
  ...
  const onEnrollCaptured = async (
    ...
    toast({
      ...
      description: "AI ro‘yxatdan o‘tkazdi. Endi Face ID bilan davomat qilishingiz mumkin",
    });
  ...
  if (isLoading || !isAuthenticated) {
    return (
      <div className="flex min-h-[100dvh] items-center justify-center bg-slate-50 text-sm text-slate-600">
        {isLoading ? "Yuklanmoqda…" : "Login sahifasiga o‘tilmoqda…"}
      </div>
    );
  }
