...
function parseDescriptorList(body: { descriptors?: unknown; descriptor?: unknown }): number[][] {
  if (Array.isArray(body?.descriptors)) {
    return (body.descriptors as unknown[])
      .map(parseFaceDescriptor)
...
const MAX_SNAPSHOT_CHARS = 420_000;
...
router.post("/auth/face/enroll", requireAuth, async (req: AuthRequest, res): Promise<void> => {
  ...
  if (descriptors.length < 2) {
    res.status(400).json({ error: "Yuz aniq olinmadi — kameraga qarab qayta urinib ko‘ring" });
    return;
  }
...
router.post("/auth/face/login", async (_req, res): Promise<void> => {
  res.status(403).json({
    error: "Face ID bilan kirish o‘chirilgan. Avval login va parol bilan kiring, keyin Davomatda Face ID ishlating.",
