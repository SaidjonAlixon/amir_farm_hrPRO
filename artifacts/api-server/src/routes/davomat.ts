...
import { evaluateLiveness, matchFaceForUserWithAi, type LivenessProof } from "../lib/face-match";
...
async function matchFaceForSessionUser(
  userId: number,
...
> {
  const matched = await matchFaceForUserWithAi(userId, descriptor, snapshot);
  if (!matched.ok) {
    return {
      ok: false,
      error: matched.error,
...
async function resolveFaceAtSite(opts: {
  /** Login qilingan akkaunt — yuz faqat shu egaga solishtiriladi */
...
> {
  const [user] = await db
    ...
    .from(usersTable)
    .where(eq(usersTable.id, opts.sessionUserId))
    .limit(1);
  if (!user || (user.status !== "active" && user.status !== "on_leave")) {
    return { ok: false, status: 403, body: { error: "Profil faol emas", code: "user_inactive" } };
  }

  const matched = await matchFaceForSessionUser(
    user.id,
    opts.descriptor,
    opts.snapshot,
  );
  if (!matched.ok) {
    const status = matched.code === "face_not_account_owner" ? 403 : 401;
    return {
      ok: false,
      status,
      body: {
        error: matched.error,
        code: matched.code,
        fullName: matched.fullName ?? user.fullName,
      },
    };
  }
...
/**
 * Face ID tasdiq — login majburiy.
 * Jonli yuz faqat sessiya egasining enroll qilingan yuzi bilan solishtiriladi.
 * Hali Keldim/Ketdim yozilmaydi.
 */
router.post("/davomat/face-verify", requireAuth, async (req: AuthRequest, res): Promise<void> => {
  try {
...
/**
 * Face ID davomat — avval login/parol, keyin shu akkaunt yuzi.
 * GPS hududi → Keldim / Ketdim.
 */
router.post("/davomat/face-punch", requireAuth, async (req: AuthRequest, res): Promise<void> => {
  try {
    const sessionUserId = req.userId!;
