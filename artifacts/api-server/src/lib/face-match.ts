...
> {
  ...
  const fullName = (await ownerNameOfUser(userId)) ?? undefined;
  ...
  const notOwner = (detail?: string) => ({
    ...
    error:
      ...
      (fullName
        ...
        : "Siz emassiz. Bu akkaunt egasining yuzi bilan mos kelmadi."),
  ...
  if (isFaceAiEnabled()) {
    ...
    if (!ai.ok) {
      if (ai.code === "face_ai_uncertain" || ai.code === "face_ai_low_confidence") {
        return {
          ok: false,
          error: ai.error || "Yuz aniq o‘qilmadi. Kameraga tik qarab qayta urinib ko‘ring.",
          code: ai.code,
          fullName,
        };
      }
